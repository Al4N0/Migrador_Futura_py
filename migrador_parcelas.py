...
class MigradorParcelas:
    """
    Migra os registros de CONTA_PARCELA e CAIXA_ITEM (Firebird) para a tabela `parcela` (MySQL).

    Performance: usa pré-carregamento similar aos itens.
    """

    # Colunas da tabela `parcela` no MySQL (na ordem do INSERT)
    # De acordo com a query do usuário, temos:
    # idvenda, idloja, parcela, idplano, idforma, taxid, idcliente, idusuario, data, documento, pixcopiacola, prazo, vencimento, valor, valorfinal, valorrecebido, created, quitado
    _COLUNAS_MYSQL = [
        "idvenda", "idloja", "parcela", "idplano", "idforma", "taxid",
        "idcliente", "idusuario", "data", "documento", "pixcopiacola",
        "prazo", "vencimento", "valor", "valorfinal", "valorrecebido",
        "created", "quitado"
    ]
    ...
    def _carregar_mapeamento_pagamento(self):
        ...
        if os.path.exists(caminho):
            try:
                ...
                self.log(f"> Mapeamento de formas de pagamento carregado em parcelas ({len(self._mapping_pagamento)} itens).")
            except Exception as e:
                self.log(f"⚠️ Erro ao carregar mapping_pagamento.json em parcelas: {e}")
        else:
            self.log("> Nenhum mapeamento de formas de pagamento encontrado para parcelas.")
    ...
    def pre_carregar(self, cursor_fb) -> int:
        ...
        SELECT * FROM (
            ...
            WHERE P.STATUS NOT IN ('1','3') AND P.FK_EMPRESA = {self.fk_empresa}
            AND (
        ...
        for row in cursor_fb.fetchall():
            ...
            forma_origem = linha.get("forma_origem")
            mapped_val = self._mapping_pagamento.get(forma_origem)
            if mapped_val is not None:
                if isinstance(mapped_val, dict):
                    linha["idplano"] = mapped_val.get("idplano", 0)
                    linha["idforma"] = mapped_val.get("idforma", 0)
                else:
                    linha["idplano"] = mapped_val
                    linha["idforma"] = mapped_val
            else:
                linha["idplano"] = 0
                linha["idforma"] = 0
    ...
    def inserir_por_venda(
        ...
        if self._cache:
            ...
            return True # Fallback without cache not implemented yet to keep it simple, given the complex SQL
    ...
    def _montar_valores(self, parcela: dict, idvenda_mysql: int) -> tuple:
        ...
        for col in self._COLUNAS_MYSQL:
            if col == "idloja":
                ...
                valores.append(parcela.get(col, None))
